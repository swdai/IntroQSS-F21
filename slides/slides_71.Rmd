...
--
(before, we referred to *estimators*, which are a class of statistics that we use to approximate specific estimands).
...
What about the sample variance? We'll implement the same process with our simulations.
...
But *this estimate is itself a random variable*, with, again, its own sampling distribution. We will get slightly different estimates of the sampling variance of the sample mean each time we take oru `r n_iter` separate samples. 
