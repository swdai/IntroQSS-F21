...
$$
f_X(X) = \textrm{P}(X = x) \sum_y \textrm{P}(X=x, Y=y) = \sum_y f(x,y)
$$
...
$$
f_{Y|X}(y|x) = \textrm{P}(Y = y | X = x) = \frac{\textrm{P}(X=x, Y=y)}{\textrm{P}(X=x)}  = \frac{f(x,y)}{f_X(x)} 
$$
--
...
Suppose we observe the following data from the joint distribution of $X$ and $Y$. 
--
We are given that $\textrm{Var}[X] = 3$ and $\textrm{Var}[Y] = 6$. 
--
...
---
<p style="color:red">Following content if time, or in handout.</p>
...
- Discrete random variables have non-zero mass on specific points, but for continuous random variables, $\textrm{P}(X = x) = 0$. Instead of mass, we refer to *density* for continuous variables. [2]
