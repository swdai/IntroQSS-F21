...
---
We used the difference in means estimator previously to get an estimate of the average treatment effect, $\textrm{E}[\tau_i]$. 
...
- But what can we say about our uncertainty about this estimate? The sample size is pretty small. 

--

- Is it meaningfully different from zero?
...
|             | $Y_i(0)$ | $Y_i(1)$  | $D_i$ | $Y_i$ | $\tau_i^0$ |
|-------------|----------|-----------|-------|-------|----------|
| Village 1   | 15        | 15        | .blue[0]     | 15    | 0        |
| Village 2   | 15       | 15         | .blue[1]     | 15    | 0        |
| Village 3   | 20       | 20         | .blue[0]     | 20    | 0        |
| Village 4   | 20       | 20         | .blue[0]     | 20    | 0        |
| Village 5   | 10       | 10         | .blue[0]     | 10    | 0        |
| Village 6   | 15       | 15         | .blue[1]     | 15    | 0        |
| Village 7   | 30        | 30        | .blue[0]     | 30    | 0        |
--
...
|-------------|----------|-----------|-------|-------|----------|
| Village 1   | 15        | 15        | .blue[0]     | 15    | 0        |
| Village 2   | 15       | 15         | .blue[0]     | 15    | 0        |
| Village 3   | 20       | 20         | .blue[1]     | 20    | 0        |
| Village 4   | 20       | 20         | .blue[0]     | 20    | 0        |
| Village 5   | 10       | 10         | .blue[0]     | 10    | 0        |
| Village 6   | 15       | 15         | .blue[0]     | 15    | 0        |
| Village 7   | 30        | 30        | .blue[1]     | 30    | 0        |
...
df <- tibble(Y = c(15, 15, 20, 20, 10, 15, 30),
             Y0 = Y,
             Y1 = Y,
             D = c(1, 0, 0, 0, 0, 0, 1))
df
...
The mean under our null distribution is approximately zero. Why?
...
--
(before, we referred to *estimators*, which are statistics we use to approximate specific estimands).

--

*Test statistics* are the statistics we use to test a hypothesis. 
...
This is why we talk about probability AND statistics. 
...
As we said, our difference in means estimator is a function of the data we observe. 
...
But we DO know the distribution would be under the null. 
...
Then we can use our randomization inference function again. 
...
ggplot(gg_bins, aes(x = bin_min, y = n)) +
  geom_col() +
  geom_vline(xintercept = 0, color = 'red')
```
...
ggplot(gg_bins, aes(x = bin_min, y = n, fill = col)) +
  geom_col() +
  geom_vline(xintercept = 0, color = 'red') + 
  scale_fill_manual(values=c("grey35", "#619CFF")) +
  theme(legend.position = 'none')
...
declaration <- declare_ra(N = length(df$D), m = sum(df$D == 1))
...
---
```{r}
difference_in_means(Y~D, data = df)
```

--
```{r}
lm_robust(Y~D, data = df)
```
...
ggplot(gg_bins, aes(x = bin_min, y = n, fill = col2)) +
  geom_col() +
  geom_vline(xintercept = 0, color = 'red') + 
  scale_fill_manual(values=c("grey35", "#619CFF")) +
  theme(legend.position = 'none')
