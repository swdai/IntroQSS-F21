...
tb_1d <- df %>% 
    group_by(year) %>% 
    summarise('two_party_vote_share' = sum(dem_vote, na.rm = TRUE) / (sum(dem_vote, na.rm = TRUE) + sum(rep_vote, na.rm = TRUE)))
...
tb_1e <- df %>% 
    ...
    group_by(year) %>% 
    summarize(prop = mean(demwin, na.rm = TRUE))
...
plot_1f <- tb_1f %>% 
    ggplot(aes(x = two_party_vote_share, y = prop)) +
    geom_point() +
    geom_path() +
...
# your code here 
tb_1g <- tb_1f %>% 
     rename('Democratic vote share' = two_party_vote_share, 'Democratic county share' = prop) %>% 
...
```{r}
# Objective: trying to find the number of events in which a Chinese political 
# elite who got demoted yet (re)promoted afterwards
# The original dataset is in Simplified Chinese, so I have added a recoding step
# to translate
...
df_tidy <- df_ori %>% 
    ...
    rename('name' = 姓名,
           ...
           'exper_num' = 经历序号, 
           'province' = 地方一级关键词) %>% 
    # recode the position from character into a numeric ranking
    ...
    # create a new variable to indicate if the elite has been demoted and promoted afterwards
