...
(pval <- mean(abs(null_dist)>abs(ate)))
```
...
```{r}
length(y_poisson)/(length(y_poisson)-1) *mean( (y_poisson - mean(y_poisson))^2 )
...
```{r}
y_list <- map(1:1000, ~ rpois(n = 100, lambda = 10))
```
...
**This is a simulation that illustrates how the sample mean varies across i.i.d. draws from the same population distribution. The estimated standard deviation of the sample means across draws should look similar to your answers in 2d (the true value of the standard error of the sample mean) and 2e (our bootstrapped estimate of the standard error of the sample mean).**

```{r}
y_means <- map(y_list, mean)
