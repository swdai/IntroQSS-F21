...
library(tidyverse)
data_path <- './../data/gdp_per_cap/Data.csv'
```

## Getting started

Today, we are going to work on loading and tidying data. Step 1 is to download the following zip file from the course git repository "gdp_per_cap.zip". Save it in a place that you can locate it with a relative file path. Use "getwd()" if you are unsure what your working directory is.  
...
```{r}
gdp_per_cap <- read_csv(data_path)
```
...
```{r}
gdp_per_cap_tidy <- gdp_per_cap %>%
  pivot_longer(cols= '1995 [YR1995]':'2015 [YR2015]', names_to = "Year", values_to = 'gdppc')
```
...
gdp_per_cap_tidy <- gdp_per_cap_tidy %>% 
  separate(Year, into = c('Year', NA), sep = ' ')
```
...
gdp_per_cap_tidy_subset <- gdp_per_cap_tidy %>%
  group_by(_______) %>%
  _______(!all(______(gdppc))) 

```{r}
gdp_per_cap_tidy_subset <- gdp_per_cap_tidy %>%
  group_by(country) %>% #if you don't group_by, then no change would been done at all
  filter(!all(is.na(gdppc))) 
```
...
gdp_per_cap_change <- gdp_per_cap_tidy_subset %>%
  group_by(country) %>%
  arrange(country, year, by_group = TRUE) %>%
  mutate(rate = 100 * (gdppc - lag (gdppc)) /lag(gdppc)) %>%
  ungroup()
...
```{r}
plot <- gdp_per_cap_change %>% 
  ggplot(aes(x = rate)) + 
  geom_histogram(binwidth = 1) # binwidth matches with the scale on x
plot
```
