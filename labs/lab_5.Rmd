...
df <- car_data %>%
    group_by(am) %>% 
    summarize(diff_in_means = mean(mpg))
...
# how to compare? check if two values are very close to rounding error
near(my_mpg, car_data$mpg)
mean(as.logical(near(my_mpg, car_data$mpg)))
...
```{r}
car_data <- car_data %>% 
    mutate(sport = ifelse(disp > 250, 1, 0))
```
...
*YOUR CODE HERE*
```{r}
model <- estimatr::lm_robust(formula = mpg ~ am + sport, data = car_data) %>% 
    summary()
model
```
...
interactive: only when both are present
diff-in-diff for either variable thought as treatment  
...
separate data generating process of export and import  
